...
export function analyzeWeatherWindow(day: DayData, startHour: number, endHour: number): WeatherSummary {
  if (!day || !day.hours || day.hours.length === 0) {
    return { minTemp: 0, maxTemp: 0, avgPrecipProb: 0, maxWindSpeed: 0, recommendation: 'Mixed', message: 'No data available', points: [] };
  }
  ...
  if (windowHours.length === 0) {
    return { minTemp: 0, maxTemp: 0, avgPrecipProb: 0, maxWindSpeed: 0, recommendation: 'Mixed', message: 'No data for time slot', points: [] };
  }
  ...
  const recommendation = getRecommendation(minTemp, maxTemp, avgPrecipProb, maxWindSpeed);
  const message = generateMessage(minTemp, maxTemp, avgPrecipProb, maxWindSpeed);
...
export function getRecommendation(minTemp: number, maxTemp: number, precip: number, wind: number): RecommendationSignal {
...
export function generateMessage(minTemp: number, maxTemp: number, precip: number, wind: number): string {
  if (precip > 50) {
